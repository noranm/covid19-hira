...
statin.confounder <- setdiff(statin.confounder, c("anitihypertensive.use", "statin2", "Lipid.lowering.agents.including.statin", "Antineoplastic", "anti.arrhythmias.use", "Other.arrhythmias"))
...
Anticoagulants.confounder <- setdiff(Anticoagulants.confounder, c("statin", "anitihypertensive.use", "anti.arrhythmias.use", "Other.arrhythmias", "Antineoplastic"))
...
after.exposure <- function(drug, rule_mid, data, Gluc=FALSE){

  if (Gluc) {
    code_info <- data.frame("code_val" = code_ATC_df3[code_ATC_df3$use == drug, "code"],
                            "uniqueCode" = code_ATC_df3[code_ATC_df3$use == drug,"uniqueCode"],
                            "code_len" = nchar(code_ATC_df3[code_ATC_df3$use == drug, "code"]),
                            "reg" = paste0("^", code_ATC_df3[code_ATC_df3$use == drug, "code"]))
    
    search.reg <- paste(code_info$reg, collapse ="|")
    abs <- grep(search.reg, ATC_mapping_df$ATC, ignore.case = TRUE)
    mapped_var <- toupper(ATC_mapping_df[abs, "GNL_CD"])
    
    sF_split1 <- strsplit(code_ATC_df3[code_ATC_df3$use == drug,"searchFor"], "; ")[[1]]
  } else {  
...
statin120$A.statin <- statin_outcomes$statin
...
Anticoagulants120$A.Anticoagulants <- Anticoagulants_outcomes$Anticoagulants
...
immunosuppressant_outcomes$Glucocorticoid <- after.exposure(drug="Systemic Corticosteroids", rule_mid=rule_immunosuppressant2, data=immunosuppressant_outcomes, Gluc=TRUE)

immunosuppressant120$A.immunosuppressant <- immunosuppressant_outcomes$immunosuppressant
immunosuppressant120$A.Glucocorticoid <- immunosuppressant_outcomes$Glucocorticoid
...
A.Glucocorticoid120 <- immunosuppressant120[( immunosuppressant120$A.immunosuppressant == 0) | ( immunosuppressant120$A.Glucocorticoid == 1 ),]
...
A.Glucocorticoid120 <- hos.immunosuppressant120[( hos.immunosuppressant120$A.immunosuppressant == 0) | ( hos.immunosuppressant120$A.Glucocorticoid == 1 ),]
