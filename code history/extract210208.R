...
codebook2 <- "code-books-210202-2.xlsx"
...
cat(sprintf("\nlength - STATIN hospital JID : %d\n", length(intersect(age40JID, hospitalJID)) ))
...
ps.weight <- function(exposure, name, ps, h=0.06, ps.cut=0.01){
  ...
  wt.check <- function(wt){
    # (1) 1000 이 넘어가는 wt 가 한 개라도 존재하면 에러 발생.
    rule1 <- sum(wt > 1000)
    if (rule1) stop(e)
...
statin90 <- make_df(como_df = como_info_statin90, ATC_df = ATC_med_statin90,
                    outcome_df=covid_outcomes2, study = "statin90")

# [2] statin 120
print(" <-------------- statin 120 --------------> \n")
statin120 <- make_df(como_df = como_info_statin120, ATC_df = ATC_med_statin120,
                     outcome_df=covid_outcomes2, study = "statin120")
...
sink("./resultsCNLLS/model/[Before]results_statin120.txt")
cat("\n <--------- [statin 120] Result (primary) ---------> \n")
print(nrow(statin120))
print(statin.study[,c(2,1)])
summary(statin120.fit)
statin120.wt <- ps.weight(ps=statin120.fit$fitted.values, exposure=statin120$statin, name="[Before]statin120")
cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.model(Y="outcome1", wt_df=statin120.wt, df=statin120, study=statin.study, name="[Before]statin120") # 0.6249

outcome.model(Y="Death", wt_df=statin120.wt, df=statin120, study=statin.study, name="[Before]statin120")
outcome.model(Y="Use.of.mechanical.ventilation", wt_df=statin120.wt, df=statin120, study=statin.study, name="[Before]statin120")
outcome.model(Y="ICU.admission", wt_df=statin120.wt, df=statin120, study=statin.study, name="[Before]statin120")
outcome.model(Y="myocardial.infarction.ischemic.stroke.and.TIA", wt_df=statin120.wt, df=statin120, study=statin.study, name="[Before]statin120")

outcome.model(Y="Use.of.mechanical.ventilation.ICU.admission", wt_df=statin120.wt, df=statin120, study=statin.study, name="[Before]statin120")

outcome.model(Y="re.outcome1", wt_df=statin120.wt, df=statin120, study=statin.study, name="[Before]statin120") # 0.6249

print_table(df.all=statin120, wt=statin120.wt$iptw.wt, df.study=statin.study, save_name="[Before]iptw.statin120")
print_table(df.all=statin120, wt=statin120.wt$trim.wt, df.study=statin.study, save_name="[Before]trim.statin120")
print_table(df.all=statin120, wt=statin120.wt$st.iptw.wt, df.study=statin.study, save_name="[Before]st.iptw.statin120")
print_table(df.all=statin120, wt=statin120.wt$iptw.att, df.study=statin.study, save_name="[Before]iptw.att.statin120")
print_table(df.all=statin120, wt=statin120.wt$trim.att, df.study=statin.study, save_name="[Before]trim.att.statin120")
print_table(df.all=statin120, wt=statin120.wt$st.iptw.att, df.study=statin.study, save_name="[Before]st.iptw.att.statin120")
print_table(df.all=statin120, wt=statin120.wt$dec.wt, df.study=statin.study, save_name="[Before]dec.statin120")
print_table(df.all=statin120, wt=statin120.wt$match.wt, df.study=statin.study, save_name="[Before]match.statin120")
print_table(df.all=statin120, wt=statin120.wt$kernel.wt, df.study=statin.study, save_name="[Before]kernel.statin120")
sink()
...
sink("./resultsCNLLS/model/[Before]results_Anticoagulants120.txt")
...
# --> (계획) error 발생해도 진행되도록.
Anticoagulants120.wt <- ps.weight(ps=Anticoagulants120.fit$fitted.values, exposure=Anticoagulants120$Anticoagulants, name="[Before]Anticoagulants120")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.model(Y="outcome1", wt_df=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[Before]Anticoagulants120") # 0.6249

outcome.model(Y="Death", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[Before]Anticoagulants120")
outcome.model(Y="Use.of.mechanical.ventilation", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[Before]Anticoagulants120")
outcome.model(Y="ICU.admission", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[Before]Anticoagulants120")
outcome.model(Y="Use.of.mechanical.ventilation.ICU.admission", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[Before]Anticoagulants120")

outcome.model(Y="re.outcome1", wt_df=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[Before]Anticoagulants120") # 0.6249

cat("\n\n -- PRINT -- \n\n")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$iptw.wt, df.study=Anticoagulants.study, save_name="[Before]iptw.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$trim.wt, df.study=Anticoagulants.study, save_name="[Before]trim.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$st.iptw.wt, df.study=Anticoagulants.study, save_name="[Before]st.iptw.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$iptw.att, df.study=Anticoagulants.study, save_name="[Before]iptw.att.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$trim.att, df.study=Anticoagulants.study, save_name="[Before]trim.att.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$st.iptw.att, df.study=Anticoagulants.study, save_name="[Before]st.iptw.attAnticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$dec.wt, df.study=Anticoagulants.study, save_name="[Before]dec.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$match.wt, df.study=Anticoagulants.study, save_name="[Before]match.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$kernel.wt, df.study=Anticoagulants.study, save_name="[Before]kernel.Anticoagulants120")
...
outcome.model(Y="bleeding.transfusion", wt_df=Anticoagulants120.wt1, df=Anticoagulants120, study=Anticoagulants.study1, name="[Before]Anticoagulants120_1")
...
outcome.model(Y="myocardial.storke.TIA.Thromboembolism", wt_df=Anticoagulants120.wt2, df=Anticoagulants120, study=Anticoagulants.study2, name="[Before]Anticoagulants120_2")
sink()
