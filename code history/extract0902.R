...
codebook_path <- "./"
codebook <- "code-books-200903.xlsx"
...
frequency3 <- function(df){
  for (EachName in colnames(df)) {
    cat(sprintf(" --- %s --- \n",EachName))
    print(class(df[,EachName]))
    print(typeof(df[,EachName]))
    print(mode(df[,EachName]))
    print(table(df[,EachName]))
    cat("\n\n")
  }
}
...
CI.modeling <- function(df, study, name){
  ...
  for (v in study.Effect.Modifier){
    ...
    write.csv(Coef_3_v, paste0(save_path, E.mod))
...
print("check point - function check -")

###################################
############ 함수 체크 ############
###################################
df <- anti30; study <- anti.study; name <- "anti30"
sink("./resultsCNLLS/model/check_0902.txt")
...
cat("\nExposure : \n"); cat(study.exposure); cat("\n")
cat("Confounder : \n"); cat(study.confounder); cat("\n")
cat("Effect Modifier : \n"); cat(study.Effect.Modifier); cat("\n")
cat("Second : \n"); cat(study.second); cat("\n\n")
