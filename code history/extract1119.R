...
targetJID <- unique(co19_t200[(co19_t200$CONFIRM=="Y") & (!is.na(co19_t200$CONFIRM)), ]$JID)
age40JID <- unique(co19_t200[co19_t200$PAT_AGE >= 40, ]$JID)
...
# -------------------------------------------------- #
co19_t200_trans_dn <- co19_t200[(co19_t200$JID %in% targetJID), c("JID", "MID", "INSUP_TP_CD", "SEX_TP_CD", "RECU_FR_DD", "PAT_AGE", "MAIN_SICK", "SUB_SICK", "DEATH", "CONFIRM")]
...
co19_twjhe200_trans_dn <- co19_twjhe200[co19_twjhe200$JID %in% targetJID, c("JID", "MID", "SEX_TP_CD", "RECU_FR_DD", "PAT_AGE", "MAIN_SICK", "SUB_SICK", "RECU_FR_DD", "PRCL_SYM_TP_CD")]
...
# co19_t200_trans_dn
co19_T200DT <- setDT(co19_t200_trans_dn)
patient_dt <- co19_T200DT[, .(first_covid=min(RECU_FR_DD)), by=.(JID, INSUP_TP_CD, SEX_TP_CD, PAT_AGE)]
...
table(patient_info$GENDER)
table(patient_info$MedInfo)
sink()
...
CI.modeling <- function(df, study, name){
  ...
  for (outcome2 in study.second) {
    ...
    if (outcome2 %in% c("myocardial.storke.TIA.Thromboembolism", "bleeding.transfusion")){ 
      ...
      cat(sprintf("n (<=0.01) : %d, n (>=0.99) : %d\n", nrow(df[df$ps2 <= 0.01, ]), nrow(df[df$ps2 >= 0.99, ])) )
      cat(sprintf("n (<=0.05) : %d, n (>=0.95) : %d\n", nrow(df[df$ps2 <= 0.05, ]), nrow(df[df$ps2 >= 0.95, ])) )
      cat(sprintf("n (<=0.1) : %d, n (>=0.9) : %d\n", nrow(df[df$ps2 <= 0.1, ]), nrow(df[df$ps2 >= 0.9, ])) )
      cat("\n\n")
      cat(" - weight dist\n\n")
    ...
    Coef_0.2 <- data.frame("coef" = exp(summary(fit0.2)$coef[,1]), # 계수  
                           ...
                           "lower.ci" = exp(summary(fit0.2)$coef[,1] - 1.96 * summary(fit0.2)$coef[,2]), # CI
                           "upper.ci" = exp(summary(fit0.2)$coef[,1] + 1.96 * summary(fit0.2)$coef[,2]))
    write.csv(Coef_0.2, paste0(save_path, secondary_MultiNaive))
...
anti90$V_cancer <- NULL
anti120$V_cancer <- NULL
...
immune90$V_cancer <- NULL
immune120$V_cancer <- NULL 
...
antico90 <- anti90[( anti90$Anticoagulants.Antiplatelets == 1) | ( anti90$ex.Antiplatelets == 0 ),]
antico120 <- anti120[( anti120$Anticoagulants.Antiplatelets == 1) | ( anti120$ex.Antiplatelets == 0 ),]
...
antico90$wt2 <- ( 1/antico90$ps2 ) *antico90$ex.Anticoagulants + ( 1/( 1-antico90$ps2 ) )*( 1-antico90$ex.Anticoagulants )
sink()
...
antipl90 <- anti90[( anti90$Anticoagulants.Antiplatelets == 1) | ( anti90$ex.Anticoagulants == 0 ),]
antipl120 <- anti120[( anti120$Anticoagulants.Antiplatelets == 1) | ( anti120$ex.Anticoagulants == 0 ),]
...
gluc90 <- immune90[( immune90$immunosuppressant == 1) | ( immune90$Glucocorticoid == 0 ),]
gluc120 <- immune120[( immune120$immunosuppressant == 1) | ( immune120$Glucocorticoid == 0 ),]
...
print_table <- function(df.all, df.study, Bdays){
  ...
  v.vec3 <- m.vec3*(1-m.vec3)
  v.vec4 <- m.vec4*(1-m.vec4)
  atc.aSD1 <- cal.aSD(m.vec3, m.vec4, v.vec3, v.vec4)
  
  # weighted prevalance 's sd?
  mm.vec3 <- ATCTable$AfterIPTW.User / sum(df.User$wt)
  mm.vec4 <- ATCTable$AfterIPTW.NonUser / sum(df.NonUser$wt)
  vv.vec3 <- mm.vec3*(1-mm.vec3)
  vv.vec4 <- mm.vec4*(1-mm.vec4)
  atc.aSD2 <- cal.aSD(mm.vec3, mm.vec4, vv.vec3, vv.vec4)
