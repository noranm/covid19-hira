...
co19_t200$AGE = 2020 - as.numeric(substr(co19_t200$PAT_BTH,1,4))
co19_t200DT <- data.table(co19_t200)

targetJID <- intersect(unique(co19_t200DT[CONFIRM =="Y",]$JID), unique(co19_t200DT[AGE >= 18, ]$JID))
age40JID <- unique(co19_t200DT[AGE >= 40, ]$JID)

# -------------------------------------------------- #
co19_t200_trans_dn <- co19_t200[co19_t200$JID %in% targetJID, c("JID", "MID", "INSUP_TP_CD", "SEX_TP_CD", "RECU_FR_DD", "AGE", "MAIN_SICK", "SUB_SICK", "DEATH", "CONFIRM")]
...
co19_twjhe200_trans_dn <- co19_twjhe200[co19_twjhe200$JID %in% targetJID,c("JID", "MID", "SEX_TP_CD", "RECU_FR_DD", "PAT_AGE", "MAIN_SICK", "SUB_SICK", "RECU_FR_DD")]
co19_twjhe200_trans_dn$RECU_FR_DD <- as.Date(as.character(co19_twjhe200_trans_dn$RECU_FR_DD), "%Y%m%d")

co19_twjhe300_trans_dn <- co19_twjhe300[co19_twjhe300$JID %in% targetJID,c("JID", "MID", "DIV_CD")]
...
# co19_t200_trans_dn
patient_dt <- as.data.table(co19_t200_trans_dn)[, .(first_covid = min(RECU_FR_DD)), by = .(JID, INSUP_TP_CD, SEX_TP_CD, AGE)]
patient_info <- as.data.frame(patient_dt)

agebreaks2 <- c(0,19,40,65,500)
agelabels2 <- c("0-18","19-39", "40-64","65+")

setDT(patient_info)[, AGE2 := cut(AGE,
                                  breaks = agebreaks2,
                                  right = FALSE, 
                                  labels = agelabels2)]
...
summary(patient_info[,c("INSUP_TP_CD2", "AGE2", "SEX_TP_CD2")])
...
#####################################################
cat("=======================================================\n")
cat("================ 함수 체크 [1] outcomes ===============\n")
cat("=======================================================\n")

codeBook <- code_outcomes_df; study = NULL; Bdays = 0
...
for (i in 1:nrow(codeBook)) {
  ...
  if (verbose) cat("[check] 4 \n")
  if (!is.null(study)) {
    if (verbose) cat("[check] 4 - 1 (study not null) \n")
    r.v <- codeBook[i, study]
    #if (verbose) cat(sprintf(" - %s in %s.\n\n", r.v, study))
  ...
  if (verbose) cat("[check] 5 \n")
  if (codeBook[i,"codeType"] == "ATC") {
    if (verbose) cat("[check] 5 - 1 (ATC code)\n")
    
    rule = (toupper(substr(ATC_mapping_df[ ,"ATC"],1,code_len)) == toupper(code_val))
    #--------- (if -2-) "ATC" mapping X ---------#
    if (sum(rule)==0){ 
      if (verbose) cat("[check] next (no ATC mapping) \n")
      # cat(sprintf(" ATC code %s : mapping X\n", uniqueCode ))
      next
    }
  }
  if (verbose) cat("[check] 6 \n")
  sF_split1 <- strsplit(codeBook[i,"searchFor"], "; ")
  ...
  while(!is.na(sF_split1[[1]][j])){
    ...
    if (is.null(df_rule)){
      ...
      if (r.v %in% names(df_rule)){ # names(df_rule) : JID, MID, EXPOSURE.itself, exposure.and.outcome.related
        if (verbose) cat("[check] 7 - df_rule exist\n")
        df <- df_all[df_all$MID %in% df_rule[df_rule[,r.v]==1,"MID"], ]
      }
      else{
        df <- df_all
    ...
    if (codeBook[i,"codeType"] == "ATC") {
      if (verbose) cat("[check] 7 - ATC\n")
      mapped_var <- toupper(ATC_mapping_df[rule, var_name])
      searchJID <- df[ifelse(toupper(df[,var_name]) %in% mapped_var, TRUE, FALSE),]$JID
      
    } else { #--------- (else -1-) codeBook$codeType != "ATC" ---------#
      if (verbose) cat("[check] 7 - not ATC \n")
      searchJID <- df[ifelse(toupper(substr(df[,var_name],1,code_len)) == toupper(code_val), TRUE, FALSE),]$JID
      
    }
    j = j+1
    if (!length(searchJID)) {
      if (verbose) cat("[check] next (no JID) \n\n")
      next()
    }
    final[final$JID %in% searchJID, uniqueCode] <- 1
    
  } # while loop exit
  if (verbose) cat("[check] 7 - finish \n")
}
if (verbose) cat("[check] 8 - finish \n\n")
# [!] 추가함
final[is.na(final)] <- 0
covid_outcomes <- data.frame(final)
...
codeBook <- code_ATC_df; study = "statin.study"; Bdays = 30
...
if (Bdays!=0) {
  for (jid in patient_info$JID) {
    ...
    if (nrow(df_temp)<=20) cat(sprintf("[check] - nrow(df_temp) = %d \n", nrow(df_temp)))
    if (!nrow(df_temp)) next
    ...
    df_rule <- rbind(df_rule, df_rule_temp)
    if (nrow(df_temp)<=20) cat(sprintf("[check] 1 - 2 \n"))
    df_temp <- NULL
    # print(colSums(df_rule[df_rule$JID ==jid, c(3,4)]))
  }
  cat("[check] 1 - finish \n")
...
sink()
#######################
save.image(file = "./Rdata0817.RData")
...
anti.confounder <- c("AGE", "GENDER", "MedInfo", gsub("[-,/ ]", ".", anti.Ex.Out.Rel), gsub("[-,/ ]", ".", anti.Out.Rel))[-55]
...
immune.confounder <- c("AGE", "GENDER", "MedInfo", gsub("[-,/ ]", ".", immune.Ex.Out.Rel), gsub("[-,/ ]", ".", immune.Out.Rel))[-55]
