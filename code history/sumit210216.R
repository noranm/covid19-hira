...
# flow chart N #
sink("./resultCNNLS/flow-chart.txt")
cat(sprintf("\n TEST : %d\n", length(testJID) ))
#cat(sprintf("\n TEST & under 40 ages : %d\n", length(testJID) ))
#cat(sprintf("\n TEST & under 20 ages : %d\n", length(testJID) ))
...
outcome.model <- function(Y="outcome1", wt_df, df, study, name){
  ...
  if (!is.null(wt_df$trim.att)) {
    df.trim <- df[wt_df$trim.att!=0,]
    Exposure.trim <- Exposure[wt_df$trim.att!=0]
    df.User2 <- df.trim[Exposure.trim==1,]
    df.NonUser2 <- df.trim[Exposure.trim==0,]
    print(sum(wt_df$trim.att==0))
    OutcomeTable.T <- data.frame(Index=c("NonUser", "User"),
                                 N_Patients=c(nrow(df.NonUser2), nrow(df.User2)),
                                 N_Events=c(sum(df.NonUser2[, Y]),sum(df.User2[, Y])),
                                 EventRates=c(sum(df.NonUser2[, Y])/nrow(df.NonUser2), sum(df.User2[, Y])/nrow(df.User2) ) )
    print(OutcomeTable.T); cat("\n")
    outcome_path.T <- paste0(save_path, Y, "_Incidence_TRIMMING.att.csv")
    write.csv(OutcomeTable.T, outcome_path.T)
  }
...
statin_outcomes2 <- statin_outcomes[statin_outcomes$JID %in% age40JID, ]
statin_outcomes2$outcome1 <- apply(statin_outcomes2[,-1],1,min)

date_info_statin <- merge(date_info[date_info$JID %in% age40JID,], statin_outcomes2[,c("JID", "outcome1")], all.x=TRUE, by="JID")
...
statin_outcomes2$statin <- after.exposure(drug="statin", 
                                          rule_mid=rule_statin2, 
                                          data=statin_outcomes2)
...
como_after_statin120 <- como_after_statin120[como_after_statin120$JID %in% age40JID, ]
...
ATC_after_statin120 <- ATC_after_statin120[ATC_after_statin120$JID %in% age40JID, ]
...
A.statin120$statin <- statin_outcomes2$statin
...
# dataset check
sink("./resultsCNLLS/after-d-check1.txt")
cat("\n\n ** STATIN 120 ** \n\n")
...
A.statin120$V_cancer <- 0 ; A.statin120[A.statin120$JID %in% age40.cancer_jid,]$V_cancer <- 1
