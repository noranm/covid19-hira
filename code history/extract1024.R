...
immune90 <- immune90[( immune90$immunosuppressant == 1) | ( immune90$Glucocorticoid == 0 ),]
immune120 <- immune120[( immune120$immunosuppressant == 1) | ( immune120$Glucocorticoid == 0 ),]

sink("./resultsCNLLS/model/results_Glucocorticoid90.txt")
cat("\n <--------- [ Glucocorticoid 90 ] Result (primary) ---------> \n")
immune90$ps2 <- CI.modeling(df = immune90, study=gluc.study[order(gluc.study$type, gluc.study$var),], name="Glucocorticoid90")
immune90$wt2 <- ( 1/immune90$ps2 ) *immune90$Glucocorticoid + ( 1/( 1-immune90$ps2 ) )*( 1-immune90$Glucocorticoid )
sink()

# Glucocorticoid 120
sink("./resultsCNLLS/model/results_Glucocorticoid120.txt")
cat("\n <--------- [ Glucocorticoid 120 ] Result (primary) ---------> \n")
immune120$ps2 <- CI.modeling(df = immune120, study=gluc.study[order(gluc.study$type, gluc.study$var),], name="Glucocorticoid120")
immune120$wt2 <- ( 1/immune120$ps2 ) *immune120$Glucocorticoid + ( 1/( 1-immune120$ps2 ) )*( 1-immune120$Glucocorticoid )
sink()
