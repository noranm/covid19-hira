...
patient_info$MedAid <- ifelse(patient_info$INSUP_TP_CD == 5,1,0)
...
make_df <- function(como_df, ATC_df, study = "statin30") {
  ...
  # ** 수정 ** "Lipid_lowering_agents_including_statin"
...
# code_ICD10_df : exposure.and.outcome.related(confoudner), outcome.related, x, x.exposure.only
# code_ATC_df : exposure.and.outcome.related(confounder), EXPOSURE.itself, outcome.related
anti.con.como <- unique(code_ICD10_df[code_ICD10_df$antiplatelet.anticoagulant.study == "exposure.and.outcome.related",
                                  c("name", "antiplatelet.anticoagulant.study")])$name
anti.con.med <-  unique(code_ATC_df[code_ATC_df$antiplatelet.anticoagulant.study == "exposure.and.outcome.related",
                                c("name", "antiplatelet.anticoagulant.study")])$name
anti.confounder <- c(anti.con.como, anti.con.med)

anti.exposure <- "Anticoagulants/antiplatelets"
# 변동가능성 우선, effect modifier 만 포함.
anti.regression_predictor <- c("AGE", # (1) AGE
                               "GENDER", # (2) GENDER
...
# code_ICD10_df : exposure.and.outcome.related(confoudner), outcome.related, x, x.exposure.only
# code_ATC_df : exposure.and.outcome.related(confounder), EXPOSURE.itself, outcome.related
immune.con.como <- unique(code_ICD10_df[code_ICD10_df$immune.suppressant.study == "exposure.and.outcome.related",
                                        c("name", "immune.suppressant.study")])$name
immune.con.med <- unique(code_ATC_df[code_ATC_df$immune.suppressant.study == "exposure.and.outcome.related",
                                     c("name", "immune.suppressant.study")])$name
immune.confounder <- c(immune.con.como, immune.con.med)
immune.exposure <- "immunosuppressant"

# 변동가능성 우선, effect modifier 만 포함.
immune.regression_predictor <- c("AGE", # (1) AGE
...
sink()
###############
como_info_statin30 <- NULL
como_info_statin120 <- NULL
como_info_anti30 <- NULL
como_info_anti120 <- NULL
como_info_immune30 <- NULL
como_info_immune120 <- NULL

ATC_med_statin30 <- NULL
ATC_med_statin120 <- NULL
ATC_med_anti30 <- NULL
ATC_med_anti120 <- NULL
ATC_med_immune30 <- NULL
ATC_med_immune120 <- NULL

statin30$Lipid_lowering_agents_including_statin <- statin30[,"Lipid lowering agents including statin"]
mytable(Lipid_lowering_agents_including_statin~AGE+GENDER+MedAid+MedInsurance, data=statin30)
names(statin30)
