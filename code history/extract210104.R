...
targetJID <- unique(co19_t200[(co19_t200$CONFIRM=="Y") & (co19_t200$AGE_cal > 19), ]$JID)
...
# -> error 기록용
sink("./resultsCNLLS/Check.txt")
age40JID <- unique(co19_t200_trans_dn[(co19_t200_trans_dn$AGE_cal >= 40), "JID"])
...
hospitalMID <- c( co19_t200_trans_dn[(FOM_TP_CD2 %in% c("021", "071")), "MID"] )
hospital <- unique(co19_t200_trans_dn[(co19_t200_trans_dn$MID %in% hospitalMID), "JID"])
hospitalJID <- intersect(hospital, targetJID)
...
cancer_jid1 <- unique(co19_twjhe200_trans_dn[co19_twjhe200_trans_dn$PRCL_SYM_TP_CD %in% c("V027", "V193", "V194"), ]$JID)
cancer_jid2 <- unique(co19_t200_trans_dn[co19_t200_trans_dn$PRCL_SYM_TP_CD %in% c("V027", "V193", "V194"), ]$JID)
cancer_jid <- unique(c(cancer_jid1, cancer_jid2))

print(length(cancer_jid))

age40.cancer_jid <- intersect(cancer_jid, age40JID)
print(length(age40.cancer_jid))

sink()

# External files (code_book)
codebook_path <- "./"
codebook <- "code-books-210202.xlsx"
...
patient_dt <- co19_T200DT[, .(first_visit=min(RECU_FR_DD)), by=.(JID, SEX_TP_CD, AGE_cal, INSUP_TP_CD)]
...
patient_info <- as.data.frame(patient_info[,c("JID", "first_visit", "AGE_cal", "AGE", "GENDER", "MedInfo")])
...
sink("./resultsCNLLS/processing_log.txt")
...
make_df <- function(como_df, ATC_df, outcome_df, study) {
  ...
  #como_df_agg <- cbind(como_df_agg1, como_df_agg2[,!colnames(como_df_agg2) %in% colnames(como_df_agg1)])
  ...
  #ATC_df_agg <- cbind(ATC_df_agg1, ATC_df_agg2[,!colnames(ATC_df_agg2) %in% colnames(ATC_df_agg1)])
...
table(immunosuppressant90$immunosuppressant.for.subanalysis)
table(Anticoagulants120$immunosuppressant)
...
outcome.model <- function(Y="outcome1", wt_df, df, study, name){
  ...
  outcome_path.T <- paste0(save_path, Y, "_Incidence_TRIMMING.csv")
...
dist_check <- function(df.all, wt.df, study, save_name){
  save_path <- sprintf("./resultsCNLLS/ps/[Check]distribution/%s", save_name)
  dir.create(save_path, showWarnings=FALSE)
  ...
  print(dist0)
  print(dist1)
  subdist0 <- NULL; subdist1 <- NULL
  for (v in study.Effect.Modifier) {
    df.NonUser_v0 <- df.NonUser[df.NonUser[,v]==0,]
    df.NonUser_v1 <- df.NonUser[df.NonUser[,v]==1,]
    
    df.User_v0 <- df.User[df.User[,v]==0,]
    df.User_v1 <- df.User[df.User[,v]==1,]
    
    for (o in c("outcome1", study.outcome2)) {
      t0_v0 <- as.matrix(table("wt"=round(df.NonUser_v0$ps,4), df.NonUser_v0[, o]))
      t0_v1 <- as.matrix(table("wt"=round(df.NonUser_v1$ps,4), df.NonUser_v1[, o]))
      
      t1_v0 <- as.matrix(table("wt"=round(df.User_v0$ps,4), df.User_v0[, o]))
      t1_v1 <- as.matrix(table("wt"=round(df.User_v1$ps,4), df.User_v1[, o]))
      
      
      subdist0 <- cbind(subdist0, t0_v0)
      subdist0 <- rbind(subdist0, t1_v0)
      
      subdist1 <- cbind(subdist0, t0_v1)
      subdist1 <- rbind(subdist0, t1_v1)
    }
  }
  print(subdist0)
  print(subdist1)
...
statin120.wt <- ps.weight(ps=statin120.fit$fitted.values, exposure=statin120$statin, name="statin120")
cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.model(Y="outcome1", wt_df=statin120.wt, df=statin120, study=statin.study, name="statin120") # 0.6249

outcome.model(Y="Death", wt_df=statin120.wt, df=statin120, study=statin.study, name="statin120")
outcome.model(Y="Use.of.mechanical.ventilation", wt_df=statin120.wt, df=statin120, study=statin.study, name="statin120")
outcome.model(Y="ICU.admission", wt_df=statin120.wt, df=statin120, study=statin.study, name="statin120")
outcome.model(Y="myocardial.infarction.ischemic.stroke.and.TIA", wt_df=statin120.wt, df=statin120, study=statin.study, name="statin120")

outcome.model(Y="Use.of.mechanical.ventilation.ICU.admission", wt_df=statin120.wt, df=statin120, study=statin.study, name="statin120")

outcome.model(Y="re.outcome1", wt_df=statin120.wt, df=statin120, study=statin.study, name="statin120") # 0.6249

print_table(df.all=statin120, wt=statin120.wt$iptw.wt, df.study=statin.study, save_name="iptw.statin120")
print_table(df.all=statin120, wt=statin120.wt$trim.wt, df.study=statin.study, save_name="trim.statin120")
print_table(df.all=statin120, wt=statin120.wt$st.iptw.wt, df.study=statin.study, save_name="st.iptw.statin120")
print_table(df.all=statin120, wt=statin120.wt$iptw.att, df.study=statin.study, save_name="iptw.att.statin120")
print_table(df.all=statin120, wt=statin120.wt$trim.att, df.study=statin.study, save_name="trim.att.statin120")
print_table(df.all=statin120, wt=statin120.wt$st.iptw.att, df.study=statin.study, save_name="st.iptw.att.statin120")
print_table(df.all=statin120, wt=statin120.wt$dec.wt, df.study=statin.study, save_name="dec.statin120")
print_table(df.all=statin120, wt=statin120.wt$match.wt, df.study=statin.study, save_name="match.statin120")
print_table(df.all=statin120, wt=statin120.wt$kernel.wt, df.study=statin.study, save_name="kernel.statin120")
sink()
...
# --> (계획) error 발생해도 진행되도록.
Anticoagulants120.wt <- ps.weight(ps=Anticoagulants120.fit$fitted.values, exposure=Anticoagulants120$Anticoagulants, name="Anticoagulants120")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.model(Y="outcome1", wt_df=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="Anticoagulants120") # 0.6249

outcome.model(Y="Death", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="Anticoagulants120")
outcome.model(Y="Use.of.mechanical.ventilation", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="Anticoagulants120")
outcome.model(Y="ICU.admission", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="Anticoagulants120")
outcome.model(Y="Use.of.mechanical.ventilation.ICU.admission", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="Anticoagulants120")

outcome.model(Y="re.outcome1", wt_df=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="Anticoagulants120") # 0.6249

cat("\n\n -- PRINT -- \n\n")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$iptw.wt, df.study=Anticoagulants.study, save_name="iptw.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$trim.wt, df.study=Anticoagulants.study, save_name="trim.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$st.iptw.wt, df.study=Anticoagulants.study, save_name="st.iptw.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$iptw.att, df.study=Anticoagulants.study, save_name="iptw.att.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$trim.att, df.study=Anticoagulants.study, save_name="trim.att.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$st.iptw.att, df.study=Anticoagulants.study, save_name="st.iptw.attAnticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$dec.wt, df.study=Anticoagulants.study, save_name="dec.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$match.wt, df.study=Anticoagulants.study, save_name="match.Anticoagulants120")
print_table(df.all=Anticoagulants120, wt=Anticoagulants120.wt$kernel.wt, df.study=Anticoagulants.study, save_name="kernel.Anticoagulants120")
...
outcome.model(Y="bleeding.transfusion", wt_df=Anticoagulants120.wt1, df=Anticoagulants120, study=Anticoagulants.study1, name="Anticoagulants120_1")
...
Anticoagulants120.wt2 <- ps.weight(ps=Anticoagulants120.fit2$fitted.values,
                                   exposure=Anticoagulants120$Anticoagulants,
                                   name="statin120")
...
outcome.model(Y="myocardial.storke.TIA.Thromboembolism", wt_df=Anticoagulants120.wt2, df=Anticoagulants120, study=Anticoagulants.study2, name="Anticoagulants120_2")
sink()
