...
gen_dummies <- function(patient_df, codeBook, study = NULL, Bdays = 0){
  ...
  for (i in name_unique) {
    ...
    while(!is.na(sF_split1[[1]][j])){
      ...
      if (is.null(r.v)){
        df <- df_all[df_all$MID %in% df_rule[df_rule[,"all"],"MID"], ]
...
primary.CI <- function(df, study, subgroup=FALSE){
  print(study[,c(2,1)])
  ...
  #if (subgroup) {
  #  study.scr <- c()
  #  study.confounder <- p.screening(df, "outcome1", study, study.scr)
  #}
  
  # [0-1] Naive model  outcome ~ exposure : wt (X)
  fit0.1 <- glm(df[,"outcome1"]~df[,study.exposure], family="binomial")
  ...
  fit2a <- glm(df[,"outcome1"]~df[,study.exposure], weight = 1/df$wt1a, family="binomial")
  ...
  if (!subgroup){
    Effect.Mod(study.Effect.Modifier, study.exposure, df, df$wt1a)
  }
}

secondary.CI <- function(df, study, subgroup=FALSE){
  print(study[,c(2,1)])
  study.exposure <- study[study$type == "Exposure","var"]
  study.Effect.Modifier <- study[study$type == "Effect.Modifier","var"]
  ...
  for (outcome2 in study.second) {
    ...
    if (outcome2 %in% c("myocardial.storke.TIA.Thromboembolism", "bleeding.transfusion")){ 
      ...
      # (df2, outcome, study, scr)
      study.confounder <- p.screening(df, outcome2, study, study.scr)
    }
    ...
    fit1b <- glm(df[,study.exposure] ~ ., data = df[,unique(c(study.Effect.Modifier, study.confounder))], family="binomial")
    df$ps1b <- predict(fit1b, type="response")
    df$wt1b <- df$ps1b*df[,study.exposure] + (1-df$ps1b)*(1-df[,study.exposure])
    
    cat("\n - * - * - * - * - * - * - * - * - * - * - * - *\n")
    cat(" [1] Propensity Score Model (b)\n\n")
    print(summary(df$ps1b))
    print(summary(df$wt1b))
    print(summary(fit1b))
    ...
    fit2b <- glm(df[,"outcome1"]~df[,study.exposure], weight = 1/df$wt1b, family="binomial")
...
Effect.Mod <- function(Effect.V, exposure, df2, w){
  cat("\n - * - * - * - * - * - * - * - * - * - * - \n")

  for (v in Effect.V){
    cat(sprintf(" [3] Effect Modifier Model : %s \n\n", v))
    df2[,paste0(exposure,"*",v)] <- df2[,exposure] * df2[,v]
    fit3_v <- glm(df2[,"outcome1"]~., data=df2[,c(exposure, v, paste0(exposure,"*",v))], weight = 1/w, family="binomial")
...
p.screening <- function(df2, outcome, study, scr){
  confounder <- study[study$type == "Confounder","var"] 
  
  for (c in confounder[4:length(confounder)]){
    fit.scr <- glm(df2[,outcome]~df2[,c], family = "binomial")
    if ( length(summary(fit.scr)$coef) == 4) {
      print("ERROR!!, exposure coefficeints is NA.")
      next
    }
    if (summary(fit.scr)$coef[2,4] < 0.2){
      scr <- c(scr, confounder)
    }
  }
  cat("\n [Confounder after p-value Screening(0.2)] \n")
  print(study[study$var %in% scr, c(2,1)])
...
como_info_statin30 <- gen_dummies(patient_info[patient_info$JID %in%age40_jid,], code_ICD10_df, "statin.study",30)
...
primary.CI(df = statin30, study=statin.study[order(statin.study$type, statin.study$var),])
sink()

sink("./resultsCNLLS/results2_statin30.txt")
cat("\n <--------- [statin 30] Result (second) ---------> \n")
secondary.CI(df = statin30, study=statin.study[order(statin.study$type, statin.study$var),])
sink()

# statin 120
sink("./resultsCNLLS/results_statin120.txt")
cat("\n <--------- [statin 120] Result (primary) ---------> \n")
primary.CI(df = statin120, study=statin.study[order(statin.study$type, statin.study$var),])
sink()

sink("./resultsCNLLS/results2_statin120.txt")
cat("\n <--------- [statin 120] Result (second) ---------> \n")
secondary.CI(df = statin120, study=statin.study[order(statin.study$type, statin.study$var),])
sink()

# anti 30
sink("./resultsCNLLS/results_anti30.txt")
cat("\n <--------- [anti 30] Result (primary) ---------> \n")
primary.CI(df = anti30, study=anti.study[order(anti.study$type, anti.study$var),])
sink()

sink("./resultsCNLLS/results2_anti30.txt")
cat("\n <--------- [anti 30] Result (second) ---------> \n")
secondary.CI(df = anti30, study=anti.study[order(anti.study$type, anti.study$var),])
sink()

# anti 120
sink("./resultsCNLLS/results_anti120.txt")
cat("\n <--------- [anti 120] Result (primary) ---------> \n")
primary.CI(df = anti120, study=anti.study[order(anti.study$type, anti.study$var),])
sink()

sink("./resultsCNLLS/results2_anti120.txt")
cat("\n <--------- [anti 120] Result (second) ---------> \n")
secondary.CI(df = anti120, study=anti.study[order(anti.study$type, anti.study$var),])
sink()

# immune 30
sink("./resultsCNLLS/results_immune30.txt")
cat("\n <--------- [immune 30] Result (primary) ---------> \n")
primary.CI(df = immune30, study=immune.study[order(immune.study$type, immune.study$var),])
sink()

sink("./resultsCNLLS/results2_immune30.txt")
cat("\n <--------- [immune 30] Result (second) ---------> \n")
secondary.CI(df = immune30, study=immune.study[order(immune.study$type, immune.study$var),])
sink()

# immune 120
sink("./resultsCNLLS/results_immune120.txt")
cat("\n <--------- [immune 120] Result (primary) ---------> \n")
primary.CI(df = immune120, study=immune.study[order(immune.study$type, immune.study$var),])
sink()

sink("./resultsCNLLS/results2_immune120.txt")
cat("\n <--------- [immune 120] Result (second) ---------> \n")
secondary.CI(df = immune120, study=immune.study[order(immune.study$type, immune.study$var),])
sink()



### subgroup analysis
primary.CI(statin30_2[statin30_2$AGE==0,], statin.study, subgroup = TRUE)
primary.CI(statin30_2[statin30_2$AGE==1,], statin.study, subgroup = TRUE)

secondary.CI(statin30_2[statin30_2$AGE==0,], statin.study, TRUE)
