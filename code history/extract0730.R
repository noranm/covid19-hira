...
statin.confounder <- c("AGE", "GENDER", "MedInfo", gsub("[-, ]", ".", statin.Ex.Out.Rel, statin.Out.Rel))
...
anti.confounder <- c("AGE", "GENDER", "MedInfo", gsub("[-, ]", ".", anti.Ex.Out.Rel, anti.Out.Rel))
...
immune.confounder <- c("AGE", "GENDER", "MedInfo", gsub("[-, ]", ".", immune.Ex.Out.Rel, immune.Out.Rel))
...
CI.model <- function(df1, study){
  ...
  #fit1b <- glm(df1[,study.exposure] ~ ., data = df1[,unique(c(study.exposure, study.Effect.Modifier, study.confounder))], family="binomial")
