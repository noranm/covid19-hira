...
load("./resultsCNLLS/Rdata0817.RData")
...
cat("\n -- modeling -- \n")
df <- statin30 ; study <- statin.study; name <- "statin30"
...
df <- data.frame(exposure = runif(),
                 x1 = rep(0, 500),
                 x2 = rep(0, 500),
                 x3 = rep(0, 500))
glm(exposure ~ x1 + x2 + x3, data = df, family = "binomial")
