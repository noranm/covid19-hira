...
codebook <- "code-books-200605.xlsx"
...
gen_dummies <- function(codeBook, Bdays = 0){
  final <- NULL
  for (i in 1:nrow(codeBook)) {
    ...
    while(!is.na(sF_split1[[1]][j])){
      ...
      for (jid in FirstCovid19$JID){
        ...
        Bdays_rule <- df_temp$RECU_FR_DD > (FirstCovid19[FirstCovid19$JID == jid,"first_covid"]-Bdays)
        if (sum(Bdays_rule) == 0) next
        df_temp2 <- df_temp[Bdays_rule, ]
        df_mid <- rbind(df_mid, df_temp2)
      }
      df <- df_all[which(df_all$MID %in% df_mid$MID),]
      # if (sum(Bdays_rule) == 0) next # 0으로 코딩되어야 하기 때문에
      print(nrow(df))
...
# co19_t200_trans_dn
first_covid19 <- function(df) {
  new_df <- df %>% group_by(JID) %>% summarize(first_covid = min(RECU_FR_DD)) %>% data.frame()
  new_df$B30d <- new_df$first_covid-30
  new_df$B120d <- new_df$first_covid-120
  return (new_df)
}

FirstCovid19 <- first_covid19(co19_t200_trans_dn)
#twjhe200_FirstCovid19 <- first_covid19(co19_twjhe200_trans_dn)

time = Sys.time()
# [1] comorbidties ICD10 (sheet2)
# sum(phx_info) : 184(all), 149(120days), 144(30dyas)
phx_info <- gen_dummies(code_ICD_df)
phx_info30 <- gen_dummies(code_ICD10_df, 30)
phx_info120 <- gen_dummies(code_ICD10_df, 120)
sum(phx_info)
sum(phx_info30)
sum(phx_info120)
...
ATC_med30 <- gen_dummies(code_ATC_df, 30)
ATC_med120 <- gen_dummies(code_ATC_df, 120)
sum(ATC_med) # 224
sum(ATC_med30) # 12가 NA 로 코딩됨 why?
sum(ATC_med120) # 154
time3 = Sys.time()
