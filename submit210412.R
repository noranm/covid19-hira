...
codebook2 <- "code-books-210329-2.xlsx"
...
outcome.model <- function(Y="outcome1", wt_df, df, study, name){
  ...
  X <- df[,c(study.exposure, "AGE_cal", "GENDER", "MedInfo", "Hypertension", "Diabetes.mellitus")]
  ...
  for (v in study.Effect.Modifier){
    ...
    ########## REFIT Separately ##########
    if (v == "AGE") {
      study.sub.confounder <- setdiff(study.confounder, "AGE_cal")
      sub.Confounder <- df[,study.sub.confounder]
      
    } else if (v %in% c("TIA.Stroke.Coronary.Atherosclerosis.Peripheral", "TIA.Stroke.Coronary.Peripheral")) {
      study.sub.confounder <- setdiff(study.confounder, c("stroke.or.TIA.history"))
      sub.Confounder <- df[,study.sub.confounder]
      sub.Confounder$AGE_cal2 <- (sub.Confounder$AGE_cal)^2
      
    } else if (v == "immuned.deficiency.including.Cacner.HIV") {
      study.sub.confounder <- setdiff(study.confounder, c("HIV", "autoimmune.disease", "Antineoplastic", "Malignancy"))
      sub.Confounder <- df[,study.sub.confounder]
      sub.Confounder$AGE_cal2 <- (sub.Confounder$AGE_cal)^2
    } else {
...
outcome.doubly <- function(Y="outcome1", wt_df, df, study, name){
  save_path <- sprintf("./resultsCNLLS/model/%s_coef/", name)
  dir.create(save_path, showWarnings=FALSE)
  cat("\n\n ================================== \n\n")
  cat(name); cat("\n"); cat(Y)
  ...
  # [1] Outcome adjustment model
  Doubly <- data.frame(df[,doubly_cov])
  Adjust <- data.frame(A=Exposure, PS=wt_df$ps, Doubly)
  fit1 <- glm(Outcome~., data=Adjust, family="binomial")
  ...
  for (wt in wt_names){
    ...
    Doub.Exposure <- data.frame(Exposure, Doubly)
    fit2a <- glm(Outcome~.,data=Doub.Exposure, weight = wt2, family="binomial")
    ...
    tryCatch({
      cat("\n (*) GEE variance \n\n"); 
      beta <- coef(fit2a)[!is.na(coef(fit2a))]
      need <- setdiff(names(beta), "(Intercept)")
      Xmat <- as.matrix(cbind(1,Doub.Exposure[,need]))
  ...
  for (v in study.Effect.Modifier){
    ...
    for (wt in sub_wt_names){
      ...
      wt2 <- sub.wt[,wt]
      # outcome1 ~ exposure + v + exposure*v (iptw.wt)
      col <- unique(c(colnames(EffectModifier), colnames(Doubly)))
      Doub.Emodifier <- cbind(EffectModifier, Doubly)[,col]
      fit3_v <- glm(Outcome~., data=Doub.Emodifier, weight=wt2, family="binomial")
      print(summary(fit3_v)); cat("\n\n"); print(vcov(fit3_v)); cat("\n\n")
      ...
      tryCatch({
        beta <- fit3_v$coef
        beta <- beta[!is.na(beta)]; p <- length(names(beta))
        Xmat <- as.matrix(cbind(1, Doub.Emodifier[, names(beta)[2:p] ]))
        ...
        write.csv(Coef_3_v, paste0(save_path, E.mod1))
        write.csv(s, paste0(save_path, E.mod2))
        check = sum( names(beta) %in% c( "(Intercept)", study.exposure, v, "AV") )
        if (check==4){
          exp.b0 <- Coef_3_v["(Intercept)",1]; var.b0 <- s[1,1]; se.b0 <- sqrt(s[1,1])
          exp.b1 <- Coef_3_v[study.exposure,1]; var.b1 <- s[study.exposure,study.exposure]; se.b1 <- sqrt(s[study.exposure,study.exposure])
          exp.b2 <- Coef_3_v[v,1]; var.b2 <- s[v,v]; se.b2 <- sqrt(s[v,v])
          exp.b3 <- Coef_3_v["AV",1]; var.b3 <- s["AV","AV"]; se.b3 <- sqrt(s["AV","AV"])
...
doubly_cov <- c("AGE_cal", "GENDER", "MedInfo" , "Asthma", "chronic.lung.disease", "heart.failure_past", "Psychiatric.disorders", "stroke.or.TIA.history", "acetaminophen", "Antiplatelets", "Antipsychotics", "immunosuppressant", "Lipid.lowering.agents.including.statin", "arrhythmias.or.antiarrhythmias.use")
# doubly_cov <- c("AGE_cal", "Asthma", "heart.failure_past", "Psychiatric.disorders", "Antiplatelets")
...
statin.confounder <- c("AGE_cal", "GENDER", "MedInfo", gsub("[-,/ ]", ".", statin.Ex.Out.Rel), gsub("[-,/ ]", ".", statin.Out.Rel), "arrhythmias.or.antiarrhythmias.use")
statin.confounder <- setdiff(statin.confounder, c("anitihypertensive.use", "statin2", "Antineoplastic", "Lipid.lowering.agents.including.statin","anti.arrhythmias.use", "Other.arrhythmias"))
...
Anticoagulants.confounder <- c("AGE_cal", "GENDER", "MedInfo", gsub("[-,/ ]", ".", Anticoagulants.Ex.Out.Rel), gsub("[-,/ ]", ".", Anticoagulants.Out.Rel), "arrhythmias.or.antiarrhythmias.use")
Anticoagulants.confounder <- setdiff(Anticoagulants.confounder, c("statin", "anitihypertensive.use", "Antineoplastic", "anti.arrhythmias.use", "Other.arrhythmias"))
...
immunosuppressant.confounder <- c("AGE_cal", "GENDER", "MedInfo", gsub("[-,/ ]", ".", immunosuppressant.Ex.Out.Rel), gsub("[-,/ ]", ".", immunosuppressant.Out.Rel), "arrhythmias.or.antiarrhythmias.use", "immuno.status")
...
immunosuppressant90$immuned.deficiency.including.Cacner.HIV <- as.numeric( (immunosuppressant90$HIV + immunosuppressant90$autoimmune.disease + immunosuppressant90$Antineoplastic + immunosuppressant90$Malignancy) > 0)
immunosuppressant120$immuned.deficiency.including.Cacner.HIV <- as.numeric( (immunosuppressant120$HIV + immunosuppressant120$autoimmune.disease + immunosuppressant120$Antineoplastic + immunosuppressant120$Malignancy) > 0)
...
statin120.con$AGE_cal2 <- (statin120$AGE_cal)^2
...
hos.statin120.con$AGE_cal2 <- (hos.statin120.con$AGE_cal)^2
...
statin90.con$AGE_cal2 <- (statin90.con$AGE_cal)^2
...
hos.statin90.con$AGE_cal2 <- (hos.statin90.con$AGE_cal)^2
...
immunosuppressant120.con$AGE_cal2 <- (immunosuppressant120.con$AGE_cal)^2
...
hos.immunosuppressant120.con$AGE_cal2 <- (hos.immunosuppressant120.con$AGE_cal)^2
...
immunosuppressant90.con$AGE_cal2 <- (immunosuppressant90.con$AGE_cal)^2
...
hos.immunosuppressant90.con$AGE_cal2 <- (hos.immunosuppressant90.con$AGE_cal)^2
...
Glucocorticoid120.con$AGE_cal2 <- (Glucocorticoid120.con$AGE_cal)^2
...
hos.Glucocorticoid120.con$AGE_cal2 <- (hos.Glucocorticoid120.con$AGE_cal)^2
...
Glucocorticoid90.con$AGE_cal2 <- (Glucocorticoid90.con$AGE_cal)^2
...
hos.Glucocorticoid90.con$AGE_cal2 <- (hos.Glucocorticoid90.con$AGE_cal)^2
...
Anticoagulants120.con$AGE_cal2 <- (Anticoagulants120.con$AGE_cal)^2
...
Anticoagulants.scr1 <- c("AGE_cal", "GENDER", "MedInfo")
...
Anticoagulants.scr2 <- c("AGE_cal", "GENDER", "MedInfo")
for (c in Anticoagulants.confounder[4:length(Anticoagulants.confounder)]){
  # outcome2 ~ confounder 중 p-value>0.2 인 confounder 찾기.
  cat("\n", c, "\n")
...
hos.Anticoagulants120.con$AGE_cal2 <- (hos.Anticoagulants120.con$AGE_cal)^2
...
Anticoagulants90.con$AGE_cal2 <- (Anticoagulants90.con$AGE_cal)^2
...
hos.Anticoagulants90.con$AGE_cal2 <- (hos.Anticoagulants90.con$AGE_cal)^2
...
immunosuppressant_outcomes$Glucocorticoid <- after.exposure(drug="Glucocorticoid", rule_mid=rule_immunosuppressant2, data=immunosuppressant_outcomes)

print(dim(patient_info2)); print(colnames(patient_info2))

A.statin120 <- statin120; A.statin120$statin <- NULL
A.statin120$statin <- statin_outcomes$statin

A.Anticoagulants120 <- Anticoagulants120; A.Anticoagulants120$Anticoagulants <- NULL
A.Anticoagulants120$Anticoagulants <- Anticoagulants_outcomes$Anticoagulants

A.immunosuppressant120 <- immunosuppressant120; A.immunosuppressant120$immunosuppressant <- NULL
A.immunosuppressant120$immunosuppressant <- immunosuppressant_outcomes$immunosuppressant
A.immunosuppressant120$Glucocorticoid <- immunosuppressant_outcomes$Glucocorticoid
...
A.statin120.con$AGE_cal2 <- (A.statin120$AGE_cal)^2
...
hos.A.statin120$AGE_cal2 <- (hos.A.statin120$AGE_cal)^2
...
A.immunosuppressant120.con$AGE_cal2 <- (A.immunosuppressant120.con$AGE_cal)^2
...
hos.A.immunosuppressant120$AGE_cal2 <- (hos.A.immunosuppressant120$AGE_cal)^2
...
A.Glucocorticoid120.con$AGE_cal2 <- (A.Glucocorticoid120$AGE_cal)^2
...
hos.A.Glucocorticoid120.con$AGE_cal2 <- (hos.A.Glucocorticoid120.con$AGE_cal)^2
...
A.Anticoagulants120.con$AGE_cal2 <- (A.Anticoagulants120.con$AGE_cal)^2
...
hos.A.Anticoagulants120.con$AGE_cal2 <- (hos.A.Anticoagulants120.con$AGE_cal)^2
...
sink("./resultsCNLLS/model/[DOUBLY][Before][ALL]results_Anticoagulants120.txt")
cat("\n <--------- [Anticoagulants 120] Result (primary) ---------> \n")
print(nrow(Anticoagulants120))
print(Anticoagulants.study[,c(2,1)])
summary(Anticoagulants120.fit)

Anticoagulants120.wt <- ps.weight(ps=Anticoagulants120.fit$fitted.values, exposure=Anticoagulants120$Anticoagulants, name="[DOUBLY][Before][ALL]Anticoagulants120")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.doubly(Y="outcome1", wt_df=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants120") # 0.6249

outcome.doubly(Y="Death", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants120")
outcome.doubly(Y="ICU.admission", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation.ICU.admission", wt=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants120")

outcome.doubly(Y="re.outcome1", wt_df=Anticoagulants120.wt, df=Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants120") # 0.6249
...
Anticoagulants120.wt1 <- ps.weight(ps=Anticoagulants120.fit1$fitted.values,
                                   exposure=Anticoagulants120$Anticoagulants,
                                   name="[DOUBLY][Before][ALL]Anticoagulants120_1")
...
outcome.doubly(Y="bleeding.transfusion", wt_df=Anticoagulants120.wt1, df=Anticoagulants120, study=Anticoagulants.study1, name="[DOUBLY][Before][ALL]Anticoagulants120_1")
...
Anticoagulants120.wt2 <- ps.weight(ps=Anticoagulants120.fit2$fitted.values, exposure=Anticoagulants120$Anticoagulants, name="[DOUBLY][Before][ALL]Anticoagulants120_2")
...
outcome.doubly(Y="myocardial.storke.TIA.Thromboembolism", wt_df=Anticoagulants120.wt2, df=Anticoagulants120, study=Anticoagulants.study2, name="[DOUBLY][Before][ALL]Anticoagulants120_2")
sink() 
...
sink("./resultsCNLLS/model/[DOUBLY][Before][HOS]results_hos.Anticoagulants120.txt")
cat("\n <--------- [Anticoagulants 120] Result (primary) ---------> \n")
print(nrow(hos.Anticoagulants120))
print(Anticoagulants.study[,c(2,1)])
summary(hos.Anticoagulants120.fit)
...
# deciles, trimming... 
hos.Anticoagulants120.wt <- ps.weight(ps=hos.Anticoagulants120.fit$fitted.values, exposure=hos.Anticoagulants120$Anticoagulants, name="[DOUBLY][Before][HOS]hos.Anticoagulants120")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.doubly(Y="outcome1", wt_df=hos.Anticoagulants120.wt, df=hos.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants120") # 0.6249

outcome.doubly(Y="Death", wt=hos.Anticoagulants120.wt, df=hos.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation", wt=hos.Anticoagulants120.wt, df=hos.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants120")
outcome.doubly(Y="ICU.admission", wt=hos.Anticoagulants120.wt, df=hos.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation.ICU.admission", wt=hos.Anticoagulants120.wt, df=hos.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants120")

outcome.doubly(Y="re.outcome1", wt_df=hos.Anticoagulants120.wt, df=hos.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants120") # 0.6249
...
hos.Anticoagulants120.wt1 <- ps.weight(ps=hos.Anticoagulants120.fit1$fitted.values,
                                       exposure=hos.Anticoagulants120$Anticoagulants,
                                       name="[DOUBLY][Before][HOS]hos.Anticoagulants120_1")
...
outcome.doubly(Y="bleeding.transfusion", wt_df=hos.Anticoagulants120.wt1, df=hos.Anticoagulants120, study=Anticoagulants.study1, name="[DOUBLY][Before][HOS]hos.Anticoagulants120_1")
...
hos.Anticoagulants120.wt2 <- ps.weight(ps=hos.Anticoagulants120.fit2$fitted.values, exposure=hos.Anticoagulants120$Anticoagulants, name="[DOUBLY][Before][HOS]hos.Anticoagulants120_2")
...
outcome.doubly(Y="myocardial.storke.TIA.Thromboembolism", wt_df=hos.Anticoagulants120.wt2, df=hos.Anticoagulants120, study=Anticoagulants.study2, name="[DOUBLY][Before][HOS]hos.Anticoagulants120_2")
sink() 
...
sink("./resultsCNLLS/model/[DOUBLY][Before][ALL]results_Anticoagulants90.txt")
cat("\n <--------- [Anticoagulants 120] Result (primary) ---------> \n")
print(nrow(Anticoagulants90))
print(Anticoagulants.study[,c(2,1)])
summary(Anticoagulants90.fit)
...
# --> error (count 수가 너무 적어서 sample data가 에러 발생)
# --> (계획) error 발생해도 진행되도록.
Anticoagulants90.wt <- ps.weight(ps=Anticoagulants90.fit$fitted.values, exposure=Anticoagulants90$Anticoagulants, name="[DOUBLY][Before][ALL]Anticoagulants90")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.doubly(Y="outcome1", wt_df=Anticoagulants90.wt, df=Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants90") # 0.6249

outcome.doubly(Y="Death", wt=Anticoagulants90.wt, df=Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants90")
outcome.doubly(Y="Use.of.mechanical.ventilation", wt=Anticoagulants90.wt, df=Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants90")
outcome.doubly(Y="ICU.admission", wt=Anticoagulants90.wt, df=Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants90")
outcome.doubly(Y="Use.of.mechanical.ventilation.ICU.admission", wt=Anticoagulants90.wt, df=Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants90")

outcome.doubly(Y="re.outcome1", wt_df=Anticoagulants90.wt, df=Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][ALL]Anticoagulants90") # 0.6249
...
Anticoagulants90.wt1 <- ps.weight(ps=Anticoagulants90.fit1$fitted.values,
                                  exposure=Anticoagulants90$Anticoagulants,
                                  name="[DOUBLY][Before][ALL]Anticoagulants90_1")
...
outcome.doubly(Y="bleeding.transfusion", wt_df=Anticoagulants90.wt1, df=Anticoagulants90, study=Anticoagulants.study1, name="[DOUBLY][Before][ALL]Anticoagulants90_1")
...
Anticoagulants90.wt2 <- ps.weight(ps=Anticoagulants90.fit2$fitted.values, exposure=Anticoagulants90$Anticoagulants, name="[DOUBLY][Before][ALL]Anticoagulants90_2")
...
outcome.doubly(Y="myocardial.storke.TIA.Thromboembolism", wt_df=Anticoagulants90.wt2, df=Anticoagulants90, study=Anticoagulants.study2, name="[DOUBLY][Before][ALL]Anticoagulants90_2")
sink() 
...
sink("./resultsCNLLS/model/[DOUBLY][Before][HOS]results_hos.Anticoagulants90.txt")
cat("\n <--------- [Anticoagulants 120] Result (primary) ---------> \n")
print(nrow(hos.Anticoagulants90))
print(Anticoagulants.study[,c(2,1)])
summary(hos.Anticoagulants90.fit)
...
# --> error (count 수가 너무 적어서 sample data가 에러 발생)
# --> (계획) error 발생해도 진행되도록.
hos.Anticoagulants90.wt <- ps.weight(ps=hos.Anticoagulants90.fit$fitted.values, exposure=hos.Anticoagulants90$Anticoagulants, name="[DOUBLY][Before][HOS]hos.Anticoagulants90")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.doubly(Y="outcome1", wt_df=hos.Anticoagulants90.wt, df=hos.Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants90") # 0.6249

outcome.doubly(Y="Death", wt=hos.Anticoagulants90.wt, df=hos.Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants90")
outcome.doubly(Y="Use.of.mechanical.ventilation", wt=hos.Anticoagulants90.wt, df=hos.Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants90")
outcome.doubly(Y="ICU.admission", wt=hos.Anticoagulants90.wt, df=hos.Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants90")
outcome.doubly(Y="Use.of.mechanical.ventilation.ICU.admission", wt=hos.Anticoagulants90.wt, df=hos.Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants90")

outcome.doubly(Y="re.outcome1", wt_df=hos.Anticoagulants90.wt, df=hos.Anticoagulants90, study=Anticoagulants.study, name="[DOUBLY][Before][HOS]hos.Anticoagulants90") # 0.6249
...
hos.Anticoagulants90.wt1 <- ps.weight(ps=hos.Anticoagulants90.fit1$fitted.values,
                                      exposure=hos.Anticoagulants90$Anticoagulants,
                                      name="[DOUBLY][Before][HOS]hos.Anticoagulants90_1")
...
outcome.doubly(Y="bleeding.transfusion", wt_df=hos.Anticoagulants90.wt1, df=hos.Anticoagulants90, study=Anticoagulants.study1, name="[DOUBLY][Before][HOS]hos.Anticoagulants90_1")
...
hos.Anticoagulants90.wt2 <- ps.weight(ps=hos.Anticoagulants90.fit2$fitted.values, exposure=hos.Anticoagulants90$Anticoagulants, name="[DOUBLY][Before][HOS]hos.Anticoagulants90_2")
...
outcome.doubly(Y="myocardial.storke.TIA.Thromboembolism", wt_df=hos.Anticoagulants90.wt2, df=hos.Anticoagulants90, study=Anticoagulants.study2, name="[DOUBLY][Before][HOS]hos.Anticoagulants90_2")
sink()
...
sink("./resultsCNLLS/model/[DOUBLY][After][ALL]results_A.Anticoagulants120.txt")
cat("\n <--------- [Anticoagulants 120] Result (primary) ---------> \n")
print(nrow(A.Anticoagulants120))
print(Anticoagulants.study[,c(2,1)])
summary(A.Anticoagulants120.fit)

A.Anticoagulants120.wt <- ps.weight(ps=A.Anticoagulants120.fit$fitted.values, exposure=A.Anticoagulants120$Anticoagulants, name="[DOUBLY][After][ALL]A.Anticoagulants120")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.doubly(Y="outcome1", wt_df=A.Anticoagulants120.wt, df=A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][ALL]A.Anticoagulants120") # 0.6249

outcome.doubly(Y="Death", wt=A.Anticoagulants120.wt, df=A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][ALL]A.Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation", wt=A.Anticoagulants120.wt, df=A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][ALL]A.Anticoagulants120")
outcome.doubly(Y="ICU.admission", wt=A.Anticoagulants120.wt, df=A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][ALL]A.Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation.ICU.admission", wt=A.Anticoagulants120.wt, df=A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][ALL]A.Anticoagulants120")

outcome.doubly(Y="re.outcome1", wt_df=A.Anticoagulants120.wt, df=A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][ALL]A.Anticoagulants120") # 0.6249
...
A.Anticoagulants120.wt1 <- ps.weight(ps=A.Anticoagulants120.fit1$fitted.values,
                                     exposure=A.Anticoagulants120$Anticoagulants,
                                     name="[DOUBLY][After][ALL]A.Anticoagulants120_1")
...
outcome.doubly(Y="bleeding.transfusion", wt_df=A.Anticoagulants120.wt1, df=A.Anticoagulants120, study=Anticoagulants.study1, name="[DOUBLY][After][ALL]A.Anticoagulants120_1")
...
A.Anticoagulants120.wt2 <- ps.weight(ps=A.Anticoagulants120.fit2$fitted.values, exposure=A.Anticoagulants120$Anticoagulants, name="[DOUBLY][After][ALL]A.Anticoagulants120")
...
outcome.doubly(Y="myocardial.storke.TIA.Thromboembolism", wt_df=A.Anticoagulants120.wt2, df=A.Anticoagulants120, study=Anticoagulants.study2, name="[DOUBLY][After][ALL]A.Anticoagulants120_2")
sink()
...
sink("./resultsCNLLS/model/[DOUBLY][After][HOS]results_hos.A.Anticoagulants120.txt")
cat("\n <--------- [Anticoagulants 120] Result (primary) ---------> \n")
print(nrow(hos.A.Anticoagulants120))
print(Anticoagulants.study[,c(2,1)])
summary(hos.A.Anticoagulants120.fit)
...
# --> error (count 수가 너무 적어서 sample data가 에러 발생)
# --> (계획) error 발생해도 진행되도록.
hos.A.Anticoagulants120.wt <- ps.weight(ps=hos.A.Anticoagulants120.fit$fitted.values, exposure=hos.A.Anticoagulants120$Anticoagulants, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120")

cat("\n")
cat(" Results \n") # -1.1
cat("\n")

outcome.doubly(Y="outcome1", wt_df=hos.A.Anticoagulants120.wt, df=hos.A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120") # 0.6249

outcome.doubly(Y="Death", wt=hos.A.Anticoagulants120.wt, df=hos.A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation", wt=hos.A.Anticoagulants120.wt, df=hos.A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120")
outcome.doubly(Y="ICU.admission", wt=hos.A.Anticoagulants120.wt, df=hos.A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120")
outcome.doubly(Y="Use.of.mechanical.ventilation.ICU.admission", wt=hos.A.Anticoagulants120.wt, df=hos.A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120")

outcome.doubly(Y="re.outcome1", wt_df=hos.A.Anticoagulants120.wt, df=hos.A.Anticoagulants120, study=Anticoagulants.study, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120") # 0.6249
...
hos.A.Anticoagulants120.wt1 <- ps.weight(ps=hos.A.Anticoagulants120.fit1$fitted.values,
                                         exposure=hos.A.Anticoagulants120$Anticoagulants,
                                         name="[DOUBLY][After][HOS]hos.A.Anticoagulants120_1")
...
outcome.doubly(Y="bleeding.transfusion", wt_df=hos.A.Anticoagulants120.wt1, df=hos.A.Anticoagulants120, study=Anticoagulants.study1, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120_1")
...
hos.A.Anticoagulants120.wt2 <- ps.weight(ps=hos.A.Anticoagulants120.fit2$fitted.values, exposure=hos.A.Anticoagulants120$Anticoagulants, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120_2")
...
outcome.doubly(Y="myocardial.storke.TIA.Thromboembolism", wt_df=hos.A.Anticoagulants120.wt2, df=hos.A.Anticoagulants120, study=Anticoagulants.study2, name="[DOUBLY][After][HOS]hos.A.Anticoagulants120_2")
sink() 
